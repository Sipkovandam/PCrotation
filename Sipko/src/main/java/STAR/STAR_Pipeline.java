...
public class STAR_Pipeline extends Script<STAR_Pipeline>
{
	...
	Slurm<STAR_Variables> slurmVars = new Slurm<STAR_Variables>("STAR/2.5.1b-foss-2015b");
	...
	private List<Runnable> initiate() throws CloneNotSupportedException
	{
		...
		firstBuildVars.mapperVars.setSTAR_Extra_Arguments(slurmVars.mapperVars.getSTAR_Extra_Arguments() + " --sjdbInsertSave All --limitSjdbInsertNsj 10000000");
		...
		Slurm<STAR_Variables> sTARv_FirstPass = (Slurm<STAR_Variables>) slurmVars.clone(this.getOutputFolder() + this.slurmVars.getNewJsonFN("FirstPass"));
...
//		mergeV_FirstPass.writeConfig();
		...
		secondBuildVars.mapperVars.setSTAR_Extra_Arguments(slurmVars.mapperVars.getSTAR_Extra_Arguments() + " --sjdbInsertSave All --limitSjdbInsertNsj 10000000");
		secondBuildVars.mapperVars.setSjdbFileChrStartEnd(mergeV_FirstPass.getWriteFN_Splice_2ndPassInput());//all variants used in first pass + all variants that are above the read cutoff minimum
...
//		mergeV_SecondPass.writeConfig();
		...
		geneNameAdder.setEnsgToGeneSymbolFN(this.ensgToGeneSymbolFN);
		geneNameAdder.setAnnotationFN(this.annotationFN);
		geneNameAdder.setSpliceFn(laneMerger.getSummedFn());
		...
		fileSplitterCorrectedRatios.setFn(PCApipelineLite.getWriteFolderCorrected()+"PC_1-99.txt.gz");
		...
		exonExpressionMerger.setEnsgToGeneSymbolFn(geneNameAdder.getEnsgToGeneSymbolFN());
