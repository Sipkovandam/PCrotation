...
public class PCcorrection 
{
	...
	public static void main (String[] args) throws IOException
	{
		...
		MatrixStruct[] rotationMatrixes = RotateSample.rotate(	var.sampleFile, 
																var.writeFolder, 
																var.writeFolderCorrected, 
																var.correctInputForSTdevs,
																var.log2,
																var.skipQuantileNorm,
																var.correctTotalReadCount, 
																var.spearman, 
																var.centerSamples, 
																var.setLowestToAverage,
																var.addLogVal, 
																var.rLog, 
																var.sampleFile, 
																var.chromLocationsFile, 
																var.GCgenes, var);
...
//		zScoreMatrix.write(writeFolder+"pcZscoresSamples.txt");\\\\\
										...
										String PCs) throws IOException 
	{
		...
		for(int pcs = 0; pcs < PCAadjustments.length; pcs++)//for all the different numbers of PCs to correct for (e.g. int[] PCAadjustments = new int[]{0,100,2,25,300,500,1000};};)
		{
			...
			if(adjustPCs > eigenVectors.rows())//in case the user asked for more PCs to be corrected then exist.
			...
			if(pcs != PCAadjustments.length-1 || chr21 == null)//if it is not the last batch of PC corretions
			{
