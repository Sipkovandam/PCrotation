...
public class RotateSample
{
	...
	public static MatrixStruct[] rotate(PCApipeline var) throws IOException
	{
		...
		MatrixStruct geneEigenVectors = new MatrixStruct(	eigenFile.getAbsolutePath(),
															-1,
															5001);//maximum 5001 PCs
		...
		MatrixStruct[] scoreResults = PCA.scores(	geneEigenVectors,
													singleSample,
										...
										PCApipeline var) throws IOException
	{
		...
		if (var.GCgenes != null)
		{
			...
			singleSample = GCcontent.calculateAndCorrect(	singleSample,
															gcPerGene,
															var.writeFolderCorrected + "gCperSampleWriteFN.txt",
															var.writeFolderCorrected + "GCcorrected.txt.gz");
		}
	...
	private static int getSmallestIndex(MatrixStruct expression,
										MatrixStruct singleSample,
										int r,
										int c)
	{
