...
public class PCA
{
	...
	public static void evd(	Path path,
							String eigenVectorWriteFn,
							String eigenValueWriteFn) throws IOException, NotConvergedException
	{
		log("Reading data");
		...
		DenseMatrix matrix = new DenseMatrix(	matrixSize,
												matrixSize);
		FileUtil.readMatrix(path,
							matrix);

		log("Data read");
		MatrixStruct matrixStruct = new MatrixStruct(	matrix,
														pcHeaders,
														headers);
		evd(matrixStruct,
			path,
	...
	public static MatrixStruct[] evd(	MatrixStruct matrixStruct,
										Path path) throws IOException, NotConvergedException
	{
		return evd(	matrixStruct,
					path,
					null,
					null);
	}
										...
										String eigenValueWriteFn) throws IOException, NotConvergedException
	{
		...
		DenseMatrix eigen = new DenseMatrix(evs.length,
											1);
		...
		eigenValueDecomp[0] = new MatrixStruct(	rightEigenvectors,
												headers,
												pcHeaders);
		...
		if (eigenVectorWriteFn == null)
			eigenVectorWriteFn = FileUtils.removeExtention(path.toString()) + ".eigenvectors.txt";
		eigenValueDecomp[0].write(eigenVectorWriteFn);

		log("Writing eigenvalues");
		if (eigenValueWriteFn == null)
			eigenValueWriteFn = FileUtils.removeExtention(path.toString()) + ".eigenvalues.txt";
		try (FileWriter fw = new FileWriter(eigenValueWriteFn))
		{
		...
		eigenValueDecomp[1] = new MatrixStruct(	eigen,
												pcHeaders,
												colNames);
		log("Done");
	 ...
	 * @throws IOException If cannot read from / write to disk
	 */
	public static void scores(	Path evPath,
	...
	public static MyMatrix[] scores(MyMatrix eigenVectors,
									MyMatrix expression,
	...
	public static MyMatrix[] scores(MyMatrix eigenVectors,
									...
									String originalPath,
									boolean rotateBack,
									boolean correctForAverages) throws IOException
	{
		...
		MyMatrix[] scoreResults = new MyMatrix[4];
		log("Calculating principal component scores");
		MyMatrix scoreMatrix = new MyMatrix(numRowsEV,
											numColsOri);
		System.out.println("evMatrixRows= " + eigenVectors.rows());
		System.out.println("originalMatrix= " + expression.rows());
		System.out.println("scoreMatrix= " + scoreMatrix.rows());
		System.out.println("eigenVectors= " + eigenVectors.rows());
		eigenVectors.mult(	expression,
							scoreMatrix);
		...
		scoreResults[0] = new MyMatrix(	pcHeaders,
										expression.getColHeaders(),
										scoreMatrix.values);
		...
		log("Writing scores");
		String FN = originalPath.toString().replace(".gz",
													"").replace(".txt",
																"")
				+ ".scores.txt";
		scoreResults[0].write(FN);

		//		I never use these values, so no point in calculating them
		log("Calculating Cronbach's alpha for each component");
		double[] alphas = cronbachsAlpha(	eigenVectors,
											scoreMatrix);
		log("Cronbach's alphas calculated");

		log("Writing Cronbach's alphas");
		try (FileWriter fw = new FileWriter(FileUtils.removeExtention(originalPath.toString()) + "_cronbachsAlpha.txt"))
		{
			FileUtil.writeArray(fw,
								"cronbachsAlpha",
								alphas);
		}

		scoreResults[2] = expression;
		scoreResults[3] = eigenVectors;
		return scoreResults;
	}

	private static void correctForRowAverages(	MatrixStruct expression,
												int numRowsOri,
												int numColsOri,
												DenseMatrix originalMatrix,
												String originalPath,
												String[] rowHeaders) throws IOException
	{
		...
		{
			double mean = getMean(	originalMatrix,
									r);
			for (int c = 0; c < numColsOri; c++)
			{
				originalMatrix.add(	r,
									c,
									-mean);//do the actual correction
			}
		}
		//write averages
	...
	private static void putInCorrectOrientation(MatrixStruct eigenVectors,
												MatrixStruct expression,
												boolean rotateBack)
	{
		if (rotateBack)
		{
	...
	public static void scores(	Path evPath,
								Path originalPath,
								Path averagesPath) throws IOException
	{
		...
		DenseMatrix evMatrix = new DenseMatrix(	numRowsEV,
												numColsEV);
		FileUtil.readMatrix(evPath,
							evMatrix);
		...
		DenseMatrix originalMatrix;
		if (numColsEV != numRowsOri)
		{//I had to change it, otherwise it fries my brains...
			...
			originalMatrix = new DenseMatrix(	numRowsOri,
												numColsOri);
			FileUtil.readMatrix(originalPath,
								originalMatrix,
								true);
		}
		else
		...
		MyMatrix expression = new MyMatrix(	originalMatrix,
											rowHeaders,
											colHeaders);
		MyMatrix eigenVectors = new MyMatrix(	evMatrix,
												EVrowNames,
												EVcolNames);
	...
	public static MyMatrix[] rotateBack(MyMatrix eigenVectors,
										MyMatrix expression,
										String originalPath) throws IOException
	{
		return scores(	eigenVectors,
	 ...
	 */
	private static double getMean(	DenseMatrix matrix,
									int row)
	{
		double mean = 0;
	 ...
	 */
	private static double getVariance(	DenseMatrix matrix,
										double mean,
										int row)
	{
	...
	public static void center(Path path) throws IOException
	{
		...
		DenseMatrix matrix = new DenseMatrix(	numRows,
												numCols);
		FileUtil.readMatrix(path,
							matrix);
		log("Data read");
		...
		try (FileWriter fw = new FileWriter(path.toString().replace(".gz",
																	"").replace(".txt",
																				"")
				+ ".centered.txt"))
		{
	...
	public static void scale(Path path) throws IOException
	{
		...
		for (int r = 0; r < numRows; r++)
		{
			double mean = getMean(	matrix,
									r);
			double variance = getVariance(	matrix,
											mean,
											r);
	...
	{
		return covariance(	path,
							null);
	}

	public static MatrixStruct covariance(	Path path,
											String writeName) throws IOException
	{
		...
		DenseMatrix covarianceMatrix = new DenseMatrix(	numRows,
														numRows);
		log("Calculating covariance matrix");
		for (int r1 = 0; r1 < numRows; r1++)
		{
			double mean1 = getMean(	matrix,
									r1);
			for (int r2 = r1; r2 < numRows; r2++)
			{
				double mean2 = getMean(	matrix,
										r2);
				double covariance = 0;
				for (int c = 0; c < numCols; c++)
				{
					covariance += (matrix.get(	r1,
												c)
							- mean1)
							* (matrix.get(	r2,
											c)
				...
				covarianceMatrix.set(	r1,
										r2,
										covariance);
				covarianceMatrix.set(	r2,
										r1,
										covariance);
			}
		}
		...
		if (writeName == null)
			writeName = path.toString().replace(".gz",
												"").replace(".txt",
															"")
					+ ".covariance.txt";
		...
		log("Done");
		MatrixStruct covMat = new MatrixStruct(	covarianceMatrix,
												rowHeaders,
												rowHeaders);
		return covMat;
	}
	...
	public static MatrixStruct[] correlation(Path path) throws IOException
	{
		...
		DenseMatrix correlationMatrix = new DenseMatrix(numRows,
														numRows);
		log("Calculating correlation matrix");
		for (int r1 = 0; r1 < numRows; r1++)
		{
			correlationMatrix.set(	r1,
									r1,
									1);
			double mean1 = getMean(	matrix,
									r1);
			double var1 = getVariance(	matrix,
										mean1,
										r1);
			for (int r2 = r1 + 1; r2 < numRows; r2++)
			{
				double mean2 = getMean(	matrix,
										r2);
				double var2 = getVariance(	matrix,
											mean2,
											r2);
				double covariance = 0;
				for (int c = 0; c < numCols; c++)
		...
		String writeName = path.toString().replace(	".gz",
													"").replace(".txt",
																"")
				+ ".correlation.txt";
		try (FileWriter fw = new FileWriter(writeName))
		...
		output[0] = new MatrixStruct(	correlationMatrix,
										rowHeaders,
										rowHeaders);
		return output;
	}
	...
	*/
	...
	public static MatrixStruct scores(	MatrixStruct expression,
										MatrixStruct eigenvectors,
										String scoreFn,
										boolean correctForAverages,
										int geneNumber) throws IOException
	{
		//putInCorrectOrientation(eigenVectors, expression, rotateBack);
		DenseMatrix originalMatrix = eigenvectors.matrix;
		DenseMatrix evMatrix = expression.matrix;
		int numRowsOri = eigenvectors.getRowHeaders().length;
		int numColsOri = eigenvectors.getColHeaders().length;
		int numRowsEV = expression.getRowHeaders().length;
		int numColsEV = expression.getColHeaders().length;

		MatrixStruct scoreResults = null;

		System.out.println("expression rows=" + numRowsEV);
		System.out.println("expression Cols=" + numColsEV);

		System.out.println("eigen rows=" + numRowsOri);
		System.out.println("eigen Cols=" + numColsOri);

		DenseMatrix scoreMatrix = new DenseMatrix(	numRowsEV,
													numColsOri);
		evMatrix.mult(	originalMatrix,
						scoreMatrix);

		if (geneNumber == 0)
		{
			String[] pcHeaders = new String[numColsEV];
			for (int i = 0, len = pcHeaders.length; i < len; i++)
				pcHeaders[i] = "PC" + (i + 1);
			scoreResults = new MatrixStruct(scoreMatrix,
											new String[] { expression.getRowHeaders()[0] },
											pcHeaders);
			scoreResults.write(scoreFn);//pc scores
		}
		else
		{
			scoreResults = new MatrixStruct(scoreMatrix,
	 ...
	 * @throws IOException If cannot read from / write to disk
	 */
	public static MatrixStruct transform(	MatrixStruct scores,
											MatrixStruct eigenValues,
											String scorePath) throws IOException
	{
		...
		DenseMatrix transposedEVMatrix = new DenseMatrix(	numRows,
															numCols);
		for (int comp = 0; comp < numRows; comp++)
		{
			for (int i = 0; i < numCols; i++)
			{
				double score = scoreMatrix.get(	comp,
												i);
				Double newScore = score / (Math.sqrt((numCols - 1) * eigenvalueMatrix.get(	comp,
																							0)));//I(Sipko) added -1 to numCols
				if (newScore.isNaN())
					newScore = (double) 0;
				transposedEVMatrix.set(	comp,
										i,
										newScore);
			}
		}
		...
		MatrixStruct transposed = new MatrixStruct(	transposedEVMatrix,
													rowHeaders,
													colHeaders);
		log("Done");
		return transposed;
	...
	public static void transform(	Path scorePath,
									Path eigenvectorPath) throws IOException
	{
		...
		int numCols = colHeaders.length;
		int numRows = rowHeaders.length;
		DenseMatrix scoreMatrix = new DenseMatrix(	numRows,
													numCols);
		FileUtil.readMatrix(scorePath,
							scoreMatrix);
		log("Scores read");

		log("Reading eigenvalues");
		DenseMatrix eigenvalueMatrix = new DenseMatrix(	numRows,
														1);
		FileUtil.readArray(	eigenvectorPath,
							eigenvalueMatrix,
							1);
		log("Eigenvalues read");

		MatrixStruct scores = new MatrixStruct(	scoreMatrix,
												colHeaders,
												rowHeaders);
		MatrixStruct eigenvalue = new MatrixStruct(eigenvalueMatrix);
		transform(	scores,
					eigenvalue,
					scorePath.toString().replace(	".gz",
													"").replace(".txt",
																"")
							+ ".transformedToEigenvectors.txt");
	}
	 ...
	 */
	public static double[] cronbachsAlpha(	MyMatrix evMatrix,
											MyMatrix scoreMatrix)
	{

		int numComps = evMatrix.rows();
		int len = evMatrix.cols();
		int lenScores = scoreMatrix.cols();
		...
		for (int comp = 0; comp < numComps; comp++)
		{
			...
			for (int i = 0; i < len; i++)
			{
				evSquaredSum += evMatrix.matrix.get(comp,
													i)
						* evMatrix.matrix.get(	comp,
												i);
			}
			...
			{
				scoreMean += scoreMatrix.matrix.get(comp,
													i)
						/ lenScores;
			}
			...
			for (int i = 0; i < lenScores; i++)
			{
				double score = scoreMatrix.matrix.get(	comp,
														i);
	...
	public static MatrixStruct scores1Row(	MatrixStruct expression,
											MatrixStruct eigenVectors,
											String scoreFn,
											int geneNumber) throws IOException
	{
		...
		DenseMatrix scoreMatrix = new DenseMatrix(	1,
													numRowsOri);
		scoreMatrix = mult(	evMatrix,
							originalMatrix,
							scoreMatrix);
	...
	private static DenseMatrix mult(DenseMatrix evMatrix,
									DenseMatrix originalMatrix,
									DenseMatrix scoreMatrix)
	{
		for (int i = 0; i < evMatrix.numRows(); ++i)
		{
			double dot = 0;
			for (int k = 0; k < evMatrix.numColumns(); ++k)
				dot += evMatrix.get(i,
									k)
						* originalMatrix.get(	0,
												k);
			scoreMatrix.add(0,
							i,
							1 * dot);
