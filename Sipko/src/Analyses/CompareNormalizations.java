...
import PrepData.SearchFilesInDirectories;
...
public class CompareNormalizations 
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter(new File(checkFilesFN)));
		File folder = new File(folderName);
		SearchFilesInDirectories.searchDirectory(folder, writer, "PC_1", "Smoothed101");
		writer.close();
		...
		line = null;
		int r = 0;
		while((line = reader.readLine())!= null)//each line is a file in which we want to compare the chr21 genes to the other genes
		{
			try
			{
				...
				{
					results.setRow(r, line.replace("E:\\Groningen\\Data\\PublicSamples\\Test13\\", "").replace("18DownSyndrome26Normal2Cancer_counts\\", ""), new double[]{0,0});
					r++;
					continue;
				}
					
				MatrixStruct samples = new MatrixStruct(line,-1,19);//load first 19 columns (first 19 == down syndrome)
				
				if(true)
				{
					chr21 = new MatrixStruct(chr21FN);
					chr21.keepRows1Matrix(samples);//only keep the genes on chr21 that are also in the samples & order genes same way
					System.out.println("sample.rows = " + samples.rows()+ " chr21.rows " + chr21.rows());
				}
				MatrixStruct sample = samples.getAveragesPerRow();//get averages
				...
				for(int gene = 0; gene < sample.rows(); gene++)
				{
					...
					{
	//					if(line.contains("directPCA_Rlog_0.2") && line.contains("PC_1-0_DevidedBySTdevs.txt"))
	//						System.out.println("sample.rows = " + sample.rows()+ " chr21.rows " + chr21.rows() + "gene =" + gene + " sample.matrixRows= " + sample.matrix.numRows()
	//						+ " sample.matrixCols= " + sample.matrix.numColumns() + " othersIndex " + othersIndex);
				...
				results.setRow(r, line.replace("E:\\Groningen\\Data\\PublicSamples\\Test13\\", "").replace("18DownSyndrome26Normal2Cancer_counts\\", ""), new double[]{pValue,auc});
				r++;
