...
public class MatrixStruct 
{
	...
	public MatrixStruct(int nRows, int nCols)
	{
		this.setRowHeaders(makeHeaders("Row",nRows));
	...
	public MatrixStruct(String[] rowNames, String[] colNames, double[][] expression) 
	{
	...
	public MatrixStruct(String fileName, int nRows, int nCols) 
	{
		readFile(fileName, nRows, nCols);
		
	}
	private String[] makeHeaders(String prefix, int nCols) {
	...
	{
		matrix = new DenseMatrix(expression.length, expression[0].length);
		for(int r = 0; r < expression.length; r++)
		{
			for(int c = 0; c < expression[0].length; c++)
			{
				matrix.set(r, c, expression[r][c]);
			}
		}
	}
	...
	private void readFile(String fileName, int nRows, int nCols) {
		...
		try 
		{	
			setRowHeaders(FileUtil.readRowHeaders(path,nRows));
			setColHeaders(FileUtil.readColumnHeaders(path,nCols));
	...
	public void transpose() 
	{
		...
		DenseMatrix tempMat = new DenseMatrix(matrix.numColumns(), matrix.numRows());
		
		for(int r = 0; r < matrix.numRows(); r++)
		{
			for(int c = 0; c < matrix.numColumns(); c++)
			{
				tempMat.set(c, r, matrix.get(r, c));
			}
	...
	public void addAveragesCols(MatrixStruct colAverages) 
	{
		...
		{
			double average = colAverages.matrix.get(c, 0);
			for(int r = 0; r < this.getRowHeaders().length; r++)
			{
				this.matrix.add(r, c, average);
			}
		}
	}
	public void addAveragesRows(MatrixStruct rowAverages)
	{
		for(int r = 0; r < this.getRowHeaders().length; r++)
		{
			double average = rowAverages.matrix.get(rowAverages.getRowHash().get(this.getRowHeaders()[r]), 0);
			for(int c = 0; c < this.getColHeaders().length; c++)
			{
	...
	public int countCol(int col, double cutoff)
	{
		int n = 0;
		for(int r = 0; r < this.rows(); r++)
		{
			if(this.matrix.get(r, col)> cutoff)
			n++;
		}
		return n;
	}
	public void write(String fileName) throws IOException
	{
		if(fileName.endsWith(".gz"))
		{
			...
			try (BufferedWriter fw = new BufferedWriter(new OutputStreamWriter(gzipOutputStream))) {
	            FileUtil.writeMatrix(fw, matrix, getRowHeaders(), getColHeaders());
	        }
		}
		else
...
//		}
	...
	private MatrixStruct getSubMatrix(int rows, int cols) {
		if(rows == -1 || this.rows() < rows)
			rows = this.rows();
		if(cols == -1 || this.cols() < cols)
			cols = this.cols();
		MatrixStruct subMatrix = new MatrixStruct(rows,cols);
		for(int r = 0; r < subMatrix.rows(); r++)
		{
			subMatrix.setRowHeader(r,this.getRowHeaders()[r]);
			for(int c = 0; c < subMatrix.cols(); c++)
			{
				if(r == 0)
					subMatrix.setColHeader(c, this.getColHeaders()[c]);
				subMatrix.matrix.set(r, c, this.matrix.get(r, c));
			}
		}
		return subMatrix;
	}
	...
	public void setRow(int rowNumber, String rowName, double[] rowValues) 
	{
		...
		{
			this.matrix.set(rowNumber, c, rowValues[c]);
		}
	...
	public void keepRows1Matrix(MatrixStruct sample) 
	{
		...
		for(int r = 0; r < this.rows(); r++)
		{
			if(sample.getRowHash().containsKey(this.rowHeaders[r]))
			{
				toKeep.put(this.rowHeaders[r], newPos);
				newPos++;
			}
		}
	...
	private void keepIDs(Hashtable<String, Integer> toKeep) 
	{
		MatrixStruct temp = new MatrixStruct(toKeep.size(), this.cols());
		for(int r = 0; r < this.rows(); r++)
		{
	...
	{
		MatrixStruct stDev = new MatrixStruct(this.rows(), 1);
		...
		for(int r = 0; r < this.rows(); r++)
		{
			...
			stDev.matrix.set(r, 0, standardDev);
		}
		return stDev;
	}
	public MatrixStruct stDevCols() 
	{
		MatrixStruct stDev = new MatrixStruct(this.cols(), 1);
		...
		for(int c = 0; c < this.cols(); c++)
		{
			...
			double standardDev = java.lang.Math.pow(variance,0.5);
			stDev.matrix.set(c, 0, standardDev);
		}
		return stDev;
	}
	...
	public double[] getColValues(int c) 
	{
		...
		for(int r = 0; r < this.rows(); r++)
		{
			col[r] = this.matrix.get(r, c);
		}
		return col;
	}
	public void divideBy(MatrixStruct denoms, boolean rows)
	{
		...
		if(denoms.cols()==1)//divide every column by the single column in denoms
		{
			for(int r = 0; r < this.rows(); r++)
			{
				for(int c = 0; c < this.cols(); c++)
				{
					double currentVal = this.matrix.get(r,c);
					if(rows)
					{
						this.matrix.set(r, c, currentVal/denoms.matrix.get(r, 0));
					}
					else
					{
						this.matrix.set(r, c, currentVal/denoms.matrix.get(c, 0));
					}
				}
			}
		}
	}
	public void setCol(double[] colValues, int c) 
	{
		for(int r = 0; r < this.rows(); r++)
		{
			this.matrix.set(r,c,colValues[r]);
		}
	}
	...
	public void addRandomValues(double cutoff) //adds cutoff*random() to all values <cutoff
	{
		if(cutoff <= 0)
		 return;
		...
		for(int r = 0; r < this.rows(); r++)
		{
			for(int c = 0; c < this.cols(); c++)
			{
				...
				if(value<cutoff)
					value+=generator.nextDouble()*cutoff;
				this.matrix.set(r, c,value);
			}
		}
	}
	...
	public void sortCol(int col, int opposite) 
	{
		class Row
		{
			String rowName;
		...
		for(int r = 0; r < this.rows(); r++)
		{
			rows.add(new Row(this.getRowHeaders()[r], this.getRowValues(r)));
		}
		
		Collections.sort(rows, new Comparator<Row>()
		{
			@Override
			public int compare(Row r1, Row r2)//pass this function as an argument
			{
		...
		for(int r = 0; r < this.rows(); r++)
		{
			this.setRow(r, rows.get(r).rowName, rows.get(r).values);
		}
		this.rowHash = makeHash(this.getRowHeaders());
	}
	public MatrixStruct mergeColumns(MatrixStruct addition) 
	{
		...
		{
			result.setColHeader(outC,this.getColHeaders()[c]);
			outC++;
		}
		for(int c = 0; c < addition.cols(); c++)//add addtional column headers
		{
			result.setColHeader(outC,addition.getColHeaders()[c]);
			outC++;
		}
	...
	public void setColHeader(int c, String name) 
	{
		this.colHeaders[c] = name;
		this.colHash = null;
	}
	public void setRowHeader(int r, String name) 
	{
...
//	}
	...
	{
		expressionToRank(vector, 0);
	}
	public void expressionToRank(MatrixStruct vector, double min) 
	{
		...
		for(int c = 0; c < this.cols(); c++)//sort each column
		{
			...
			{
				column[x][0] = this.matrix.get(x,c);
				column[x][1] = x;
			}
			...
			if(vector == null)
			{
				vector = new MatrixStruct(this.rows(),1);
				for(int r = 0; r < vector.rows(); r++)
					vector.matrix.set(r, 0, r);
			}
			...
			for(int r = 0; r < column.length; r++)
			{
				...
				else
				{
					...
					{
						val = (vector.matrix.get(pos,0)+vector.matrix.get(pos+1,0))/2;
					}
				}
				...
				double before = this.matrix.get((int) column[r][1],c);
				if(min != 0 && before < min)
					this.matrix.set((int) column[r][1],c,0);
				else
					this.matrix.set((int) column[r][1],c,val);		
				double after = this.matrix.get((int) column[r][1],c);	
			}
		}
	}
	public MatrixStruct quantileNormVector()
	{
		MatrixStruct sortedCols = new MatrixStruct(this.rows(), this.cols());
		...
		for(int y = 0; y < this.cols(); y++)
		{
			...
			for(int x = 0; x < this.rows(); x++)
			{
				col[x] = this.matrix.get(x,y);
			}
			Arrays.sort(col);
			
			for(int x = 0; x < this.rows(); x++)
			{
				sortedCols.matrix.set(x,y,col[x]);
			}
		}
	...
	{
		MatrixStruct qNormVector = new MatrixStruct(data.rows(), 1);
		for(int x = 0; x < data.rows(); x++)
		{
			double average = 0;
			for(int y = 0; y < data.cols(); y++)
			{
				average += data.matrix.get(x,y);
			}
			average /= data.cols();
			
			qNormVector.matrix.set(x,0,average);
		}
	...
	{
		MatrixStruct temp = new MatrixStruct(data.cols(), 1);
		temp.setRowHeaders(data.getColHeaders());
		temp.setColHeader(0, "Average");
		for(int c = 0; c < data.cols(); c++)
		{
			temp.matrix.set(c,0,data.getAverageCol(c));
		}
		return temp;
	}
	public double getAverageCol(int col)
	{
		...
		{
			if(Double.isFinite(this.matrix.get(r,col)))
				avg+= this.matrix.get(r,col);
			else
				denominator--;
		}
		if(denominator == 0)
	...
	public void adjustForAverageAllrows(MatrixStruct rowAverages) //can be called with null and will use the for the averages of the columns of this matrix
	{
		for(int r =0; r < rows(); r++)
		{
	...
	public void adjustForAverageAllCols(MatrixStruct columnAverages) //can be called with null and will use the for the averages of the columns of this MatrixStruct
	{
	...
	public void adjustForAverageCol(MatrixStruct columnAverages, int col)
	{
		double average = 0;
		if(columnAverages == null)
			average = getAverageCol(col);
		else
			average = columnAverages.matrix.get(col,0);
...
//				System.out.println("rowname" + this.getRowHeaders()[x] + " colname= "+ this.getColHeaders()[col] + " this.matrix.get(x,col)=" + this.matrix.get(x,col) + " average =" + average);
			double value = this.matrix.get(x,col);
			if(Double.isFinite(value))
				this.matrix.set(x,col, value-average);
			else
				this.matrix.set(x,col, 0);
		}
	}
	public void adjustForAverageRow(MatrixStruct rowAverages, int row)
	{
		double average = 0;
		...
		if(rowAverages != null)
		{
			...
			average = rowAverages.matrix.get(averagesRowHash.get(this.getRowHeaders()[row]),0);
		}
		else
			average = getAverageRow(row);
		for(int c = 0; c < this.cols(); c++)
		{
			double value = this.matrix.get(row,c);
			if(Double.isFinite(value))
				this.matrix.set(row,c,value-average);
			else
				this.matrix.set(row,c,0);
		}
	}
	public void removeLowExpression(String removedGenesFN, double minSamplesExpressed, double minExpression) throws IOException
	{
		...
		for(int x = 0; x < this.rows(); x++)
		{
			...
			remainder.setRow(nextRow, this.getRowHeaders()[x], this.getRowValues(x));
			nextRow++;
		}
	...
	private int[] hasVariance(int row, double minimum) 
	{
		double firstValue = this.matrix.get(row,0);
		int[] results = new int[2];
		for(int c = 0; c < this.cols(); c++)
		{
			if(firstValue != this.matrix.get(row,c));
				results[0] = 1;
			if(this.matrix.get(row,c) >= minimum)
				results[1]++;
		}
		return results;
	}
	public void correctForTotalReadCount(double endCounts, double add) 
	{
		...
		for(int c = 0; c < this.cols(); c++)
		{
			...
			for(int r = 0; r < this.rows();r++)
			{
					this.matrix.set(r,c,(this.matrix.get(r,c)+add)/total*endCounts);//(geneExpression+0.5)/totalExpresion*10^6
			}
		}
	}
...
//			removeGenes.write(writeFolder+ "RemovedForGeoMean.txt");
	...
	public void logTransform(int val, double add)//add +1 before transforming
	{
		...
		for(int x = 0; x < this.rows(); x++)
		{
			for(int y = 0; y < this.cols(); y++)
			{
				this.matrix.set(x,y,Math.log(this.matrix.get(x,y)+add)/logVal);
			}
		}
	}
	...
	public void pow(int i, double minus) 
	{
		for(int x = 0; x < this.rows(); x++)
		{
			for(int y = 0; y < this.cols(); y++)
			{
				this.matrix.set(x,y,Math.pow(10,this.matrix.get(x,y))-minus);
			}
		}
	}
	public void divideByCol(MatrixStruct denominators, int col) 
	{
		for(int c = 0; c < this.cols(); c++)
		{
			for(int r = 0; r < this.rows(); r++)
			{
				this.matrix.set(r,c,this.matrix.get(r,c)/denominators.matrix.get(c,col));
			}
		}
	}
	...
	public void removeRow(String removeGene) 
	{
		...
		MatrixStruct result = new MatrixStruct(this.rows()-1,this.cols());
		...
		for(int r = 0; r < this.rows(); r++)
		{
			...
			result.setRow(outR, this.getRowHeaders()[r], this.getRowValues(r));
			outR++;
		}
	...
	public void removeRows(MatrixStruct removeGenes) {
		...
		for(int r = 0; r < this.rows(); r++)
		{
			if(!removeGenes.getRowHash().containsKey(this.rowHeaders[r]))
			{
				toKeep.put(this.rowHeaders[r], newPos);
				newPos++;
			}
	...
	{
		MatrixStruct matRow = new MatrixStruct(this.cols(), 1);
		matRow.setRowHeaders(new String[this.cols()]);
		matRow.setColHeaders(new String[]{this.getRowHeaders()[row]});
		for(int c = 0; c < this.cols(); c++)
		{
			matRow.matrix.set(c, 0, this.matrix.get(row, c));
			matRow.setRowHeader(c, this.getColHeaders()[c]);
		}
	...
	public void divideColBy(int c, int denominator)
	{
		for(int r = 0; r < this.rows(); r++)
			this.matrix.set(r, c, this.matrix.get(r, c)/denominator);
	}
}
